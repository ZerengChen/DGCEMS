...
void NdgSWEHorizSmagrinskyDiffSolver::EvaluateDiffRHS_Nowave(double *fphys_, double *frhs_, double *fext_, int *varFieldIndex_, int*pE3d, int MyID) {
...
//#endif
		...
		UpdateViscosity_Nowave(fphys, nv_cw, pE3d, MyID);
...
/*Inner edge facial integral part*/
	Exchange(HorDiffvariable, IEFToE, pE3d, IENe, Np, K, Nfield);// use MPI to exchange 3d message
...
#pragma omp parallel for num_threads(DG_THREADS)
	...
	Exchange(HorDiffnv, IEFToE, pE3d, IENe, Np, K, 1);// use MPI to exchange 3d message
...
///***********************************************************************************************************************************************/
	...
	Exchange(HorDiffLocalPrimitiveDiffTermX, IEFToE, pE3d, IENe, Np, K, Nfield);// use MPI to exchange 3d message
	Exchange(HorDiffLocalPrimitiveDiffTermY, IEFToE, pE3d, IENe, Np, K, Nfield);// use MPI to exchange 3d message
	Exchange(HorDiffAVx, IEFToE, pE3d, IENe, Np, K, Nfield);// use MPI to exchange 3d message
	Exchange(HorDiffAVy, IEFToE, pE3d, IENe, Np, K, Nfield);// use MPI to exchange 3d message
...
void NdgSWEHorizSmagrinskyDiffSolver::UpdateViscosity_Nowave(double *fphys, double *nv_, int*pE3d, int MyID) {
	int Np = *(meshunion->cell_p->Np);
	int K = *(meshunion->K);
	double *nv_cw = nv_;
	double *hu = fphys;
	...
	for (int i = 0; i < K; i++) {
		if (MyID == pE3d[i]) {
			for (int j = 0; j < Np; j++) {
				...
				//nv_cw[i*Np + j] = 25.0;//For SaltyWater Case
			}
		}
	}
...
void NdgSWEHorizSmagrinskyDiffSolver::EvaluateDiffRHS(double *fphys_, double *frhs_, double *fext_, int *varFieldIndex_, double *time,  double *HS, double *WLEN, double *UBOT, int*pE3d, int MyID) {
	//Order of the input variable is 0 hcrit, 1 meshUnion.type, 2 prantl, 3 InnerEdge, 4 BoundaryEdge, 5 nv, 6 frhs, 7 fphys, 8 varIndex, 9 cell, 10 mesh, 11 BoundaryEdgefp
	//mexAtExit(&MyExit);
	//NdgMeshType type = meshunion->type;
	double Prantl = 1.0;//chenzereng
...
//#endif
	...
	UpdateViscosity(fphys, nv_cw, HS, WLEN, UBOT,time,pE3d,MyID);
...
void NdgSWEHorizSmagrinskyDiffSolver::UpdateViscosity(double *fphys_, double *nv_,double *HS_, double *WLEN_, double *UBOT_, double *time, int*pE3d, int MyID) {
	int Np = *(meshunion->cell_p->Np);
	int K = *(meshunion->K);
	double *WLEN = WLEN_;
	double *HS = HS_;
