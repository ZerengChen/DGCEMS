...
void InterpolationToCentralPoint(double *fphys, double *dest, int *Np2d, int *K3d, int *Np3d, double *VCV) {
	double alpha = 1;
	int Col = 1;
#ifdef _OPENMP
...
#pragma omp parallel for num_threads(DG_THREADS)
	...
	for (int p = 0; p < Np2d * K2d; p++) {
		...
		if (H2d[p] >=2 * hcrit) {
			...
			DragCoefficient[p] = pow(rr, 2);   //Cf = rr^2;

		}
		else if (H2d[p] >= hcrit) {
			rr = kappa / log((2.713 * z0b + layerHeight[p*(nlev + 1) + 1] / 2) / z0b / 2.713);
			BottomFrictionVelocity[p] = rr * sqrt(pow((huVerticalLine[p*(nlev + 1) + 1] / H2d[p]), 2) + pow((hvVerticalLine[p*(nlev + 1) + 1] / H2d[p]), 2));
			BottomFrictionLength[p] = 2.713 * z0b;
			SurfaceFrictionVelocity[p] = sqrt(pow(Taux[p], 2) + pow(Tauy[p], 2));
...
void UpdateEddyViscosity(double *fphys_, double ImplicitA_, double *fphys2d_, int Np2d, int K2d, int Np3d, int K3d, long long int nlev, double* VCV, double*BBE,double*SBE,double *Hhuv2d,int NLayer)
{
	double* h = fphys2d_;
	double* hu = fphys_;
...
void UpdateEddyViscosity_CW(double *fphys_, double ImplicitA_, double *fphys2d_, int Np2d, int K2d, int Np3d, int K3d, long long int nlev, double* VCV, double*BBE, double*SBE, double *Hhuv2d, int NLayer, double *UBOT, double *TMBOT)
{
	double* h = fphys2d_;
	...
	for (int k = 0; k < K2d; k++) {
		for (int n = 0; n < Np2d; n++) {
			//if (Hhuv2d[K2d * Np2d * 2 + k * Np2d + n] > Hcrit) {
				...
				BBE[k * Np2d + n] = 1.33 * PtrOutDragCoefficient[k * Np2d + n] * sqrt(pow(Hhuv2d[k * Np2d + n] / Hhuv2d[K2d * Np2d * 2 + k * Np2d + n], 2) + \
					pow(Hhuv2d[K2d * Np2d + k * Np2d + n] / Hhuv2d[K2d * Np2d * 2 + k * Np2d + n], 2)) * \
					Hhuv2d[k * Np2d + n] / Hhuv2d[K2d * Np2d * 2 + k * Np2d + n] * (-1);
				BBE[K2d * Np2d + k * Np2d + n] = 1.33 * PtrOutDragCoefficient[k * Np2d + n] * sqrt(pow(Hhuv2d[k * Np2d + n] / Hhuv2d[K2d * Np2d * 2 + k * Np2d + n], 2) + \
					pow(Hhuv2d[K2d * Np2d + k * Np2d + n] / Hhuv2d[K2d * Np2d * 2 + k * Np2d + n], 2)) * \
					Hhuv2d[K2d * Np2d + k * Np2d + n] / Hhuv2d[K2d * Np2d * 2 + k * Np2d + n] * (-1);
