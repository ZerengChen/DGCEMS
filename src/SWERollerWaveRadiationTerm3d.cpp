...
void SWERollerWaveRadiationTerm3d::EvaluateWaveRadiationRHS(double *fphys_, double *frhs_, double *time_, double *Hs_, double *T_, double *DIR_, double *QB_, double *WLEN_, int*pE2d, int*pE3d, int MyID)
{
...
#pragma omp parallel for num_threads(DG_THREADS)
		...
		if (MyID == pE3d[k]) {
			for (int n = 0; n < Np; n++) {
				if (Hs[k * Np + n] > 0.005) {
					...
					CFF1[k * Np + n] = KD[k * Np + n] * (1 + sigma[k * Np + n]);//CFF1 = KD(1+sigma)
					wave_C[k * Np + n] = fmax(0.0, sqrt(gra / WaveNumber[k * Np + n] * tanh(KD[k * Np + n])));
					FSS[k * Np + n] = sinh(CFF1[k * Np + n]) / (sinh(KD[k * Np + n]) + eps1);
					FCS[k * Np + n] = cosh(CFF1[k * Np + n]) / (sinh(KD[k * Np + n]) + eps1);
					FSC[k * Np + n] = sinh(CFF1[k * Np + n]) / (cosh(KD[k * Np + n]) + eps1);
					FCC[k * Np + n] = cosh(CFF1[k * Np + n]) / (cosh(KD[k * Np + n]) + eps1);
					//Rz1[k * Np + n] = cosh(2.0*sqrt(2)*3.14159265 / (Hs[k * Np + n] +eps1) * h[k * Np + n] * (1.0 + sigma[k * Np + n]));
					Rz1[k * Np + n] = 2.0 * sigma[k * Np + n] * h[k * Np + n] / (Hs[k * Np + n] + eps1);
	...
	verticalColumnIntegralField.EvaluateVerticalIntegral(Rzn, Rz1, pE2d, MyID);
...
#pragma omp parallel for num_threads(DG_THREADS)
	...
	Exchange(Roller1, IEFToE, pE3d, IENe, Np, K, 1);// use MPI to exchange 3d message
	Exchange(Roller2, IEFToE, pE3d, IENe, Np, K, 1);// use MPI to exchange 3d message
	Exchange(Roller3, IEFToE, pE3d, IENe, Np, K, 1);// use MPI to exchange 3d message
...
//	}
	...
	Exchange(H_Radiation1, IEFToE, pE3d, IENe, Np, K, 1);// use MPI to exchange 3d message
	Exchange(H_Radiation2, IEFToE, pE3d, IENe, Np, K, 1);// use MPI to exchange 3d message
	Exchange(H_Radiation3, IEFToE, pE3d, IENe, Np, K, 1);// use MPI to exchange 3d message
...
//Then, calculate the V_inneredge of inner stage.
	...
	Exchange(CFF4, IEFToE, pE3d, IENe, Np, K, 1);// use MPI to exchange 3d message
